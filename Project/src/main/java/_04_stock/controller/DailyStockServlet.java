...
public class DailyStockServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(type==null || type.trim().length()==0){
			errorMsg.add("<span>請輸入資料類型</span><br>");
		}
		if(!errorMsg.isEmpty()){
		...
		if("price".equals(type)){
			...
			String volumeData=dailyStockService.selectStockVolume(stock_Code);
			response.setContentType("application/json; charset=UTF-8");
			pw.print(volumeData);
			pw.close();
			return;
		}else if("volumeColor".equals(type)){
			String volumeColorData=dailyStockService.selectStockVolumeColor(stock_Code);
			response.setContentType("application/json; charset=UTF-8");
			pw.print(volumeColorData);
			pw.close();
			return;
		}else{
			pw.print("<span>請輸入正確的資料類型</span><br>");
			pw.close();
			return;
		}
	}
